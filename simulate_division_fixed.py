...
def birth_sizes(n: int, rng: np.random.Generator) -> np.ndarray:
    ...
    Utilisé UNIQUEMENT pour initialiser la première génération de la simulation
    de population ; la distribution stationnaire émerge ensuite naturellement
    par propagation des divisions (xb_fille = xd_mère / 2).
    """
    return rng.lognormal(BIRTH_MU, BIRTH_SIG, n)


# ═══════════════════════════════════════════════════════════════════════════════
# Simulation de population (commun aux trois modèles)
# ═══════════════════════════════════════════════════════════════════════════════

N_BURNIN_GENERATIONS = 20   # générations jetées pour atteindre le régime stationnaire


def _run_population(
    sample_division: Callable[[np.ndarray, np.random.Generator], tuple[np.ndarray, np.ndarray]],
    n: int,
    rng: np.random.Generator,
) -> pd.DataFrame:
    """
    Simule une population en propageant les divisions sur plusieurs générations.

    Principe (division symétrique, deux filles par mère) :
      1. Initialisation : n cellules avec xb tirées i.i.d. log-normale.
      2. Pour chaque génération : on applique sample_division(xb) → (xd, aud).
         Chaque mère donne deux filles dont la taille à la naissance vaut xd/2.
         On sous-échantillonne à n pour garder la population constante.
      3. Après N_BURNIN_GENERATIONS de chauffe, on collecte les n dernières cellules.

    Parameters
    ----------
    sample_division : Callable(xb, rng) → (xd, aud)
        Tire la taille et l'âge à la division étant donnée la taille de naissance xb.
    n : int
        Taille de la population cible (et taille du dataset final).
    rng : np.random.Generator

    Returns
    -------
    pd.DataFrame avec colonnes : cell_id, birth_size, division_age, division_size, increment
    """
    # ── Initialisation ────────────────────────────────────────────────────────
    xb = birth_sizes(n, rng)

    # ── Propagation sur burnin + 1 générations ────────────────────────────────
    for gen in range(N_BURNIN_GENERATIONS + 1):
        xd, aud = sample_division(xb, rng)

        if gen < N_BURNIN_GENERATIONS:
            # Deux filles par mère → 2n naissances, puis sous-échantillonnage à n
            #xb_next = np.repeat(xd / 2.0, 2)            # division symétrique
            #idx = rng.choice(len(xb_next), size=n, replace=False)
            #xb = xb_next[idx]
            xb = xd/2.0

    # ── Collecte de la génération stationnaire ────────────────────────────────
    increment = xd - xb
    return pd.DataFrame({
        ...
        "increment"    : increment.astype(np.float32),
    })
...
def simulate_age_model(rate: Rate, n: int, rng: np.random.Generator) -> pd.DataFrame:
    """
    Modèle en âge – simulation de population.

    À chaque génération, Aud est tiré depuis f(a) = B(a) exp(−∫₀^a B(s) ds)
    indépendamment de xb (le taux ne dépend que de l'âge), puis
    Xud = Xub · exp(K · Aud).  Les filles naissent à Xud / 2.
    Après N_BURNIN_GENERATIONS générations de chauffe, la distribution de xb
    converge vers la densité stationnaire de population.
    """
    def _sample(xb: np.ndarray, rng: np.random.Generator):
        aud = rate.sample_t(len(xb), rng)
        xd  = xb * np.exp(K * aud)
        return xd, aud

    return _run_population(_sample, n, rng)


def simulate_size_model(rate: Rate, n: int, rng: np.random.Generator) -> pd.DataFrame:
    """
    Modèle en taille – simulation de population.

    À chaque génération, Xud est tiré depuis f(x ; xb) = B(x) exp(−∫_{xb}^x B(s) ds)
    (dépend de la taille de naissance xb de la mère).
    Âge à la division : Aud = log(Xud / Xub) / K.
    Les filles naissent à Xud / 2.
    """
    def _sample(xb: np.ndarray, rng: np.random.Generator):
...
def simulate_incr_model(rate: Rate, n: int, rng: np.random.Generator) -> pd.DataFrame:
    """
    Modèle en incrément – simulation de population.

    À chaque génération, Z = Xud − Xub est tiré depuis f(z) = B(z) exp(−∫₀^z B(s) ds)
    indépendamment de xb (mécanisme adder pur).
    Âge à la division : Aud = log(1 + Z/Xub) / K.
    Les filles naissent à Xud / 2.
    """
    def _sample(xb: np.ndarray, rng: np.random.Generator):
        z   = rate.sample_t(len(xb), rng)
        xd  = xb + z
        aud = np.log(xd / xb) / K
        return xd, aud
